...
class Node:
    ...
    def __repr__(self) -> str:
        return f"({self.state.ego_state.x},{self.state.ego_state.y})->"
    ...
    while queue:
        ...
        for neighbour in get_neighbours(current_node):
            if is_feasible(neighbour,occupancy_map):
                ...
                target_point = Point(trajectory[current_time][0], trajectory[current_time][1])
...
def backtrack(current_node: Node): 
    ...
    while current_node:
        grid_path = [current_node.state.ego_state.x,current_node.state.ego_state.y]
